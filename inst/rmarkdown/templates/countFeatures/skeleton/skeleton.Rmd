...
output: 
  html_document: 
    keep_md: yes
params:
  axtFile: "~/projects/GenomicBreaks_results/results-v1.1.1/last/OSKA2016v1.9__Bar2_p4.axt.gz"
  gffFile: "~/projects/GenomicBreaks_results/results-v1.1.1/last/OSKA2016v1.9__Bar2_p4.gff.gz"
  id: "OSKA2016v1.9__Bar2"
  prefix: "test"
editor_options: 
...
sum(gb_col$ins)
insContext <- which(gb_col$ins)
insContext <- c(insContext -1, insContext, insContext + 1) |> unique() |> sort()
gb_col[gb_col$ins]
gb_col[insContext][1:11]
(report[["width_summary_insersions"]] <- as.list(summary(width(gb_col[gb_col$ins]))))

# Histogram of the widths
ggplot(data.frame(width = width(gb_col[gb_col$ins]))) +
  aes(width) +
  geom_histogram() +
  scale_x_log10()

# Why are the inversions overwhelmingly on the plus strand ?
ggplot(data.frame(strand = strand(gb_col[gb_col$ins]))) +
  aes(strand) +
  geom_bar()
```
