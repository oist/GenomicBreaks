...
summaryWidth <- function(gb, pasteToNames=NULL)
  customSummary(width(gb), pasteToNames)

gb$mismatches <- (width(gb) + width(gb$query) - gb$aLength - gb$matches)

report <- list() |>
  c(customSummary(gb$aLength, "aligned_length_")) |>
  c(customSummary(score(gb),  "aligned_scores_")) |>
  c(customSummary(gb$matches,                         "matches_number"))     |>
  c(customSummary(gb$mismatches,                      "mismatches_number"))  |>
  c(customSummary(gb$aLength - width(gb),             "gaps_target_"))       |>
  c(customSummary(gb$aLength - width(gb$query),       "gaps_query_"))        |>
  ...
  c(customSummary(1 - gb$mismatches / width(gb), "percentSim_compat_"))  |> # To reproduce older figures
