...
report[["aligned_target_summary"]]     <- summaryWidth(gb)
report[["aligned_query_summary"]]      <- summaryWidth(gb$query)

report[["col_reg_target_summary"]]     <- summaryWidth(coa)
report[["col_reg_query_summary"]]      <- summaryWidth(coa$query)

report[["colinear_target_summary"]]    <- summaryWidth(coli)
report[["colinear_query_summary"]]     <- summaryWidth(coli$query)

report[["isolated_target_summary"]]    <- summaryWidth(isol)
report[["isolated_query_summary"]]     <- summaryWidth(isol$query)

report[["bridge_target_summary"]]      <- summaryWidth(bri)
report[["bridge_query_summary"]]       <- summaryWidth(bri$query)

report[["break_target_summary"]]       <- summaryWidth(brk)
report[["break_query_summary"]]        <- summaryWidth(brk_q)

report[["inversions_target_summary"]]  <- summaryWidth(inv)
report[["inversions_query_summary"]]   <- summaryWidth(inv$query)

report[["transp_target_summary"]]      <- summaryWidth(tra)
report[["transp_query_summary"]]       <- summaryWidth(tra$query)

report[["guessed_target_length"]]      <- sum(guessSeqLengths(gb))
report[["guessed_query_length"]]       <- sum(guessSeqLengths(gb$query))
