...
params:
  resultsDir: "~/projects/GenomicBreaks_results/results-v1.1.1/genomicbreaks/"
  prefix: "test"
editor_options: 
...
```{r plot_tot_align}
sim <- df[c("aligned_total", "species1", "species2")]
sim <- sim[order(sim$aligned_total, decreasing = FALSE), ]
ggplot(sim) +
  aes(aligned_total, species2) +
  geom_point() +
  facet_grid(species1 ~ ., scales = "free", space = "free")
```
