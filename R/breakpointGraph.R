...
#'
#' @return The extended permutation.
#'
...
#'
#' This function computes the number of breakpoints given an extended permutation.
#'
#' @param p_extended The extended permutation.
#'
#' @return The number of breakpoints between two genomes.
#'
#' @author Bruna Fistarol
#'
#' @family Breakpoint graph functions
...
#' @param p_extended The extended permutation.
#'
#' @return Connected components of the breakpoint graph.
#'
#' @importFrom igraph make_empty_graph add_edges E V E<- V<- components induced_subgraph
#'
#' @author Bruna Fistarol
#'
#' @family Breakpoint graph functions
...
breakpointGraphProperties <- function(gb) {
  ...
  if (identical(p, seq_len(length(p)))) {
    list(N=length(p)+1, nbBreakpoints=0, nb_cycles=length(p)+1)

  # Otherwise: compute the breakpoint graph to get the number of cycles and the number of breakpoints.
  } else {
    p_extended <- extendedPermutation(p)
    bp_graph   <- breakpoint_graph(p_extended)
    list(N=length(p)+1, nbBreakpoints=bp_count(p_extended), nb_cycles=cycle_count(bp_graph))
  }
}
