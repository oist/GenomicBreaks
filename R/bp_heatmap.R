...
#'
#' This function generates a Heatmap of the specificed pattern, over breakpoints aligned at the centre of the plot. The result can be piped into \code{smoothHeatmap} and then \code{plotHeatmapList} or \code{plotHeatmapMeta}
#'
#' @param gr_ob GRanges object containing pairwise alignment
#' @param gen_seq This should be either a BSgenome object or a DNAStringSet object, such that the genome sequnce is contained in this input
#' @param basep_range range overwhich heatmap characteristic is plotted. Breakpoints will be aligned at the center of this.
...
bp_heatmap <- function(gr_ob, gen_seq, basep_range, pat){
  ...
  ind_start1 <- start(ranges(gr_ob)) < basep_range*0.5
  ind_start2 <- (start(ranges(gr_ob)) + (basep_range*0.5 -1)) > all_seq_len
  ind_end1 <- (end(ranges(gr_ob)) + (basep_range*0.5 -1)) > all_seq_len
  ind_end2 <- end(ranges(gr_ob)) < basep_range*0.5
  ...
  gr_starts1 <- start(ranges(red_gr)) - (basep_range*0.5)
  gr_starts2 <- end(ranges(red_gr)) - (basep_range*0.5)
  gr_starts <- c(gr_starts1, gr_starts2)
  gr_ends1 <- start(ranges(red_gr)) + (basep_range*0.5 -1)
  gr_ends2 <- end(ranges(red_gr)) + (basep_range*0.5 -1)
