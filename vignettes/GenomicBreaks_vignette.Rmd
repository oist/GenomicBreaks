...
```{r echo=T, message=F, warning=F}
hm_tan_O <- gr_O_Oki %>%
  tandem_coverage(tan_ref = tan_O, win = 1000, lab = "Osaka ~ Okinawa") %>%
...
```{r echo=T, message=F, warning=F}
hm_GC1 <- gr_O_Oki %>%
  bp_heatmap(gen_seq = BSgenome.Odioica.local.OSKA2016, basep_range = 1000, pat = "GC") %>%
  smoothHeatmap(output.size = c(2000,500), algorithm = "kernel")
plotHeatmap(hm_GC1)

hm_GC2 <- reduced_gr_O_Oki %>%
  bp_heatmap(gen_seq = BSgenome.Odioica.local.OSKA2016, basep_range = 1000, pat = "GC") %>%
  smoothHeatmap(output.size = c(2000,500), algorithm = "kernel") %>%
  plotHeatmap
