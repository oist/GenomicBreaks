...
```{r intersect_GBs}
intersectGBs <- function (gb1, gb2) {
  roi <- range(gb1, ignore.strand = TRUE)
  gb2[subjectHits(findOverlaps(roi, gb2))]
}
...
roi_Oki_O <- intersectGBs(roi, doubleReduced_gr_Oki_O)
roi_Oki_N <- intersectGBs(roi, doubleReduced_gr_Oki_N)
roi2 <- range(intersectGBs(roi, doubleReduced_gr_Oki_O)$query)
roi_O_N <- intersectGBs(roi2, doubleReduced_gr_O_N)
```
...
```{r genomic_features}
#zcat OKI2018_I69_1.0.gene_models.gff3.gz | awk '$7 != "-"' > /tmp/plusstrand.gff3
tx_OKI <- rtracklayer::import.gff3("/tmp/plusstrand.gff3")
tx_OKI <- GRanges(tx_OKI, seqinfo = seqinfo(OKI2018_I69))
tx_OKI <- GenomicFeatures::makeTxDbFromGRanges(tx_OKI)

hm_genes_OKI      <- feature_coverage(gr_Oki_O, feat = genes(tx_OKI),                         win = 2000, lab = "Oki genes", direction = "left")
hm_promoters_OKI  <- feature_coverage(gr_Oki_O, feat = promoters(tx_OKI),                     win = 2000, lab = "Oki promoters", direction = "left")
hm_exons_OKI      <- feature_coverage(gr_Oki_O, feat = exonicParts(tx_OKI),                   win = 2000, lab = "Oki exons", direction = "left")
hm_introns_OKI    <- feature_coverage(gr_Oki_O, feat = intronicParts(tx_OKI),                 win = 2000, lab = "Oki introns", direction = "left")
hm_cds_OKI        <- feature_coverage(gr_Oki_O, feat = cds(tx_OKI),                           win = 2000, lab = "Oki cds", direction = "left")
hm_5utrs_OKI      <- feature_coverage(gr_Oki_O, feat = unlist(fiveUTRsByTranscript(tx_OKI)),  win = 2000, lab = "Oki 5′ UTRs", direction = "left")
hm_3utrs_OKI      <- feature_coverage(gr_Oki_O, feat = unlist(threeUTRsByTranscript(tx_OKI)), win = 2000, lab = "Oki 3′ UTRs", direction = "left")
...
plotHeatmapMeta(list(hm_promoters_OKI, hm_exons_OKI, hm_introns_OKI, hm_5utrs_OKI, hm_3utrs_OKI))
