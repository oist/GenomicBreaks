...
```{r GRanges object}
gr_Osa_Oki <- load_genomic_breaks("Oidioi_pairwise_v3/OSKA2016v1.9__OKI2018_I69_1.0.gff.gz", OSKA2016v1.9, OKI2018_I69)
gr_Oki_Osa <- load_genomic_breaks("Oidioi_pairwise_v3/OKI2018_I69_1.0__OSKA2016v1.9.gff.gz", OKI2018_I69,  OSKA2016v1.9)
gr_Oki_Nor <- load_genomic_breaks("Oidioi_pairwise_v3/OKI2018_I69_1.0__OdB3.gff.gz",         OKI2018_I69,  OdB3)
gr_Osa_Nor <- load_genomic_breaks("Oidioi_pairwise_v3/OSKA2016v1.9__OdB3.gff.gz",            OSKA2016v1.9, OdB3)

# Store in a list to easier iterate on all of them.
gbs <- SimpleList(gr_Osa_Oki=gr_Osa_Oki, gr_Oki_Osa=gr_Oki_Osa, gr_Oki_Nor=gr_Oki_Nor, gr_Osa_Nor=gr_Osa_Nor)
...
gr_Osa_Oki
get_bps(gr_Osa_Oki)
get_bps(gr_Osa_Oki, direction = "left")
get_bps(gr_Osa_Oki,                      stranded = TRUE)
get_bps(gr_Osa_Oki,                      stranded = TRUE, sorted = FALSE)
get_bps(gr_Osa_Oki, direction = "right", stranded = TRUE)
get_bps(gr_Osa_Oki$query, sorted = FALSE)
get_bps(gr_Osa_Oki$query)
```
...
```{r}
sapply(list(gr_O_Oki, gr_O_N, gr_Oki_O, gr_Oki_N), synteny_index)
```

The problem with this index is that it requires genomes to be complete
...
```{r}
sapply(list(gr_O_Oki, gr_O_N, gr_Oki_O, gr_Oki_N), correlation_index)
```
