...
As previously discussed, low coverage over an alignment stop could lower the likelihood of it being considered as a breakpoint. We have per-base coverage depth information for the Okinawan genome (`Oki_cov_pb`). However, the coverage of this assembly is very good, with the lowest points of coverage still having:


```{r echo=T, message=F, warning=F}

Oki_O_ab <- get_bps(gr_ob = q_Oki) #get alignment stops (in terms of query)


min(width(ranges(Oki_cov_pb)))
