...
install.packages("BSgenome.Odioica.local.Bar2.p4", repos="https://oist.github.io/plessy_oikgenomes_drat/")

# Less contiguous genomes
install.packages("BSgenome.Odioica.local.Odioica.reference.v3.0", repos="https://oist.github.io/plessy_oikgenomes_drat/")
install.packages("BSgenome.Odioica.local.AOM.5", repos="https://oist.github.io/plessy_oikgenomes_drat/")
install.packages("BSgenome.Odioica.local.KUM.M3", repos="https://oist.github.io/plessy_oikgenomes_drat/")
```
...
gbs$Oki_Bar <- load_genomic_breaks("Oidioi_pairwise_v3/OKI2018_I69_1.0__Bar2_p4.gff.gz",      OKI2018_I69,  Bar2_p4)
gbs$Oki_Kum <- load_genomic_breaks("Oidioi_pairwise_v3/OKI2018_I69_1.0__KUM-M3-7f.gff.gz",    OKI2018_I69,  KUM_M3)
gbs$Oki_Aom <- load_genomic_breaks("Oidioi_pairwise_v3/OKI2018_I69_1.0__AOM-5-5f.gff.gz",     OKI2018_I69,  AOM_5)
...
df <- rbind(
  ...
  do.call(rbind, lapply(names(coaW1000), width2df, "collapsedW1000", coaW1000))
)
  ...
  ggplot(df) +
    ...
    geom_histogram() +
    scale_x_log10() +
    facet_wrap(~what + where, nrow = 3, scales = "free_y")
```
...
sapply(coaW1000, function(gb) sum(flagInversions(gb)$inv))
```
...
lgapsSeq_Oki_O <- BSgenome::getSeq(OKI2018_I69, inv.lgaps$Oki_Osa)
```
...
invSeq <- SimpleList()
invSeq$Oki_Osa <- BSgenome::getSeq(OKI2018_I69,  filterInv(flagInversions(coa$Oki_Osa)))
invSeq$Oki_Kum <- BSgenome::getSeq(OKI2018_I69,  filterInv(flagInversions(coa$Oki_Kum)))
invSeq$Kum_Oki <- BSgenome::getSeq(KUM_M3,  swap(filterInv(flagInversions(coa$Oki_Kum))))
invSeq$Osa_Oki <- BSgenome::getSeq(OSKA2016v1.9, filterInv(flagInversions(coa$Osa_Oki)))
invSeq$Osa_Kum <- BSgenome::getSeq(OSKA2016v1.9, filterInv(flagInversions(coa$Osa_Kum)))
...
gadems <- SimpleList()
gadems$Oki_Kum <- GADEM(invSeq$Oki_Kum[width(invSeq$Oki_Kum) < 50000], verbose=1, genome=OKI2018_I69)
...
hm_GC3 <- sapply(coaW1000, bp_heatmap, basep_range = 1000, pat = "GC", direction = "left") |> SimpleList()
...
hm_GC3_TATA <- sapply(coaW1000, bp_heatmap, basep_range = 1000, pat = "TATA", direction = "left") |> SimpleList()
...
hm_GC3_mot <- sapply(coaW1000, bp_heatmap, basep_range = 1000, pat = "AGCNGC", direction = "left") |> SimpleList()
