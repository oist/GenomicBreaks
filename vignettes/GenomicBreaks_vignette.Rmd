...
Many of the functions of the GenomicBreaks package require a GRanges object as input. So if the user wants to perform analyses on the query genome, its metadata can be accessed as follows:
...
gr_O_Oki
tol <- 1e7
reduced_gr_O_Oki <- coalesce_contigs(gr_O_Oki, tol = tol)
reduced_gr_O_Oki
reduced_gr_O_Oki <- coalesce_contigs(reduced_gr_O_Oki, tol = tol)
reduced_gr_O_Oki
reduced_gr_O_Oki <- coalesce_contigs(reduced_gr_O_Oki, tol = tol)
reduced_gr_O_Oki
...
plusMinPlus <- function (gr) {
  gr$oneNext <- precede(gr, tail(gr, -1), ignore.strand = T) - seq_along(gr) == 0
  gr$twoNext <- precede(gr, tail(gr, -2), ignore.strand = T) - seq_along(gr) == 0
...
ide <- IdeogramTrack(chromosome = "chr1", genome = "OKI2018_I69", bands =
                       data.frame(chrom="chr1", chromStart=0, chromEnd=14533022, name="chr1", gieStain="gneg"))
...
ide <- IdeogramTrack(chromosome = "S1", genome = "OSKA2016", bands =
                       data.frame(chrom="S1", chromStart=0, chromEnd=6807127, name="S1", gieStain="gneg"))
