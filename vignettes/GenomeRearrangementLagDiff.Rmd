...
# The Genome Rearrangement Problem - Analysing lag difference of unsigned permutation
...
The values for $a$ can vary according to the fragment size being inverted. So, how could we have always the same values?

The solution for this is getting the **coalescent sequence**, which means turning several colinear fragments into a single fragment. Then we can know exactly the value for the breakpoints and be able to know if the pattern is a combination of inversions. 

From now one, lets consider we are dealing with coalescent sequences.
...
```{r}
sig2unsig(-5:-1) |> inversion(4, 7) |> inversion(6, 9) |> diff() #first on the left
sig2unsig(-5:-1) |> inversion(6, 9) |> inversion(4, 7) |> diff() #first on the right
```
...
We also can have two more cases of nested inversion, when the inversions are aligned on the left or right side:

```{r}
s_unsig <- 1:10

inversion(s_unsig, 4, 7) |> inversion(4, 5) |> diff()
inversion(s_unsig, 4, 5) |> inversion(4, 7) |> diff()
inversion(s_unsig, 4, 7) |> inversion(6, 7) |> diff()
inversion(s_unsig, 6, 7) |> inversion(4, 7) |> diff()

s_unsig <- sig2unsig(-4:-1)
...
```{r}
diff_seq <- exampleDoubleInversion1 |> bpGraph() |> diff()

double1 <- c(5, 1, -4, -1, 3, -1, 4)
double2 <- c(4, -1, 3, -1, -4, 1, 5)

rearrangement_search(diff_seq, double1)
rearrangement_search(diff_seq, double2)
```
