...
```{r}
inversion(1:10, 4, 5) |> inversion(6, 7) |> inversions(2,9) |> diff()
```
...
```{r}
#return the list of required inversions to fix simple and sequential inversions, given by the lag difference search
inversions_result <- fix_simple_and_sequential(query_sequence_unsig)

seq_new <- query_sequence_unsig

len <- length(inversions_result[,1])

#update the vector with the inversions we found above
for (i in 1:len){
  seq_new <- inversion(seq_new, inversions_result[i,1], inversions_result[i,2])
}

#perform the simplified version of the algorithm
inversions_result <- rbind(inversions_result, inversions_rearrangement_simpler(new_seq))

#update the vector with the inversions we found above
for (i in (len+1):(length(inversions_result[,1]))){
  seq_new <- inversion(seq_new, inversions_result[i,1], inversions_result[i,2])
}
