...
# Simulating an Inversion

```{r Choosing the number of DNA blocks and number of random inversions}
dna_blocks <- 8
number_inversions <- 5
```
...
```{r Defining functions to simulate inversions}
...
```{r Creating the breakpoint graph}
...
```{r Construct the connected components graph}
...
components_graph <- function(g, query_sequence_unsig){
  ...
  for (i in which(cycles$csize > 1)) {
    ...
    h <- add_vertices(h, 1, 
                        cycle_index=i,
                        unoriented=any(E(cycle)$unoriented), #if it is true, the node is unoriented
...
hurdles_count <- function(g, query_sequence_unsig){
  ...
  for (i in 1:max(comps$membership)) {
    ...
    if (all(V(h)$unoriented[comp_vertices])) {
      ...
      info[i, c("min_extent", "max_extent")] <- c(u_min, u_max)
      info[i, c("contained", "contains", "hurdle", "superhurdle")] <- c(0, 0, 0, 0)
    }
  }
  ...
  #if it reaches here, line >= 2 (i.e., more than 1 unoriented component)
 ...
 hurdles <- sum(info[, "hurdles"])
  
  
 ##### calculating superhurdles ######
  
  #if the number of hurdles is even, it is not necessary to calculate superhurdles because of the definition of fortress
  
  if (hurdles%%2 == 0) {return(c(hurdles, 0))}
 
 #if all unoriented component is a hurdle, there is no hurdle protecting an unoriented component
 
  if (hurdles == line) {return(c(hurdles, 0))}
...
is_fortress <- function(hurdles){
  
  if(hurdles[1]%%2 == 0){
    return(0)
  }
  if(hurdles[1]%%2 == 1 & hurdles[1] == hurdles[2]){
    return(1)
  }
  return(0)
...
# while not all(diff(target_sequence_unsig)==1)     use when the algorithm works
...
for (loop in 1:loops){
  ...
  for (i in 1:len_qs){
    for (j in i:len_qs){
      ...
      new_hurdles <- hurdle_count(new_graph, new_seq)
      
      delta = bp_count(new_seq) - cycle_count(new_graph) + new_hurdles[1] + is_fortress(new_hurdles) 
           - (bp_count(seq) - cycle_count(bp_graph) + hurdles[1] + is_fortress(hurdles))
