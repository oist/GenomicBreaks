...
```{r}
source("vignettes/InversionsAlgorithm.R")
#install.packages("GenomicBreaks")
...
The study of genome rearrangements is an important area in computational biology that looks at how the genetic makeup of organisms has changed over time. The main question is: given two genomes and a list of ways that genomes can evolve, how can we transform one genome into another through a series of changes?
...
![Figure 3: representation of a breakpoint graph. Note that $1, 2$ and $2n+1, 2n+2$ could be connected by black and gray nodes, but by definition, the links are not created because these numbers are both consecutive and adjacent (they are being represented here, but faded) . ](breakpoint_graph.jpeg)
...
In a nutshel, hurdle is a denomination to a sets of cycles which has certain characteristics on their edges. There is another concept, **superhurdle**, the way you call a hurdle $h$ if a new hurdle is created when $h$ is removed from the graph. Then, with the number of superhurdles we can calculate a binary parameter called **fortress**. Together with the number of breakpoints and cycles, it is possible to calculate the minimal number of inversions if you have the number of hurdles and the value of fortress.
...
Here it comes a very interesting thought: lets think about probability. If we have a very large structure, it becomes less likely to have a cycle with all edges being unoriented. In fact, what happens at the end of the day is, for real data, we barely observe hurdles, because huge cycles have both unoriented and oriented edges, hence, they are oriented by definition. And this is what will save our lives later.
...
We need to find if two gray nodes overlap (i.e., their extensions overlap), but none of them contains the another one. When this happens, we say that the two gray nodes are "interleaving". If $e_1 \in C_1$ and $e_2 \in C_2$ are two gray edges interleaving, we can say $C_1$ and $C_2$ are two cycles interleaving.
...
```{r}
inversion(1:8, 4, 5) |> inversion(2,7) |> diff()
```

### Sequential inversions

The lagged difference for sequential inversion will look like:

```{r}
inversion(1:10, 4, 5) |> inversion(6, 7) |> diff()
```

Or in the opposite strand:

```{r}
inversion(1:10, 4, 5) |> inversion(6, 7) |> inversion(2,9) |> diff()
```

We can also search even for more than two fragments inverted sequentially.

Simple and sequential inversions seems to be the most common type of inversions that we can find doing a simple search.
...
Lets come back to the example from Genomic Breaks and apply this approach:

```{r}
inversions_result <- inversion_algorithm_faster(query_sequence_unsig)
new_seq <- apply_inversions(query_sequence_unsig, inversions_result)
```

This function above essentially fix simple and sequential inversions and applies the simpler version of the algorithm, which relies just on breakpoints and cycles.
...
Lets see how the sequence looks like:
...
```{r}
#analyse sequence to find non sorted regions
new_seq - 1:202 
```
We have a non sorted region starting on 34 and finishing on 52, and another one starting on 128 and finishing on 141. We are going to take these regions, plus two extra extremities (like our unsigned permutation vector) and subtract a number in order to have the sequence starting on 1. Then, apply the original algorithm and, to correct the results for the right index on the original sequence, we just have to sum the number we subtracted before.

```{r}
#these regions must start in an odd index and finish in an even index

#subtract the value of the first number to normalize the sequence

subseq_1 <- new_seq[33:52] - 32

subseq_2 <- new_seq[127:142] - 126

#then add the rearrangements to the matrix summing the subtracted value to correct the index

inversions_result <- rbind(inversions_result, (inversions_rearrangement(subseq_1) + 32))

inversions_result <- rbind(inversions_result, (inversions_rearrangement(subseq_2) + 126))

```

Now lets apply our inversions to the original sequence:

```{r}
#lets apply the rearrangements to finish the sorting
...
all(diff(new_seq))
```
The function `fix_non_sorted(seq)`, so far, is able to fix these isolated regions applying the original algorithm using the logic above.
