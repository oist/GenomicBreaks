...
new_seq - 1:202 
```
We have a non sorted region starting on 34 and finishing on 52, and another one starting on 128 and finishing on 141. We are going to take these regions, plus two extra extremities (like our unsigned permutation vector) and subtract a number in order to have the sequence starting on 1. Then, se apply the original algorithm and, to correct the results for the right index on the original sequence, we just have to sum the number we subtracted before.
